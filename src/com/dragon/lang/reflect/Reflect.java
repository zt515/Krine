...
public final class Reflect {
     ...
     * Object may be a dragon scripted object (dragon.This type).
     *
     * @return the result of the method call
     */
    public static Object invokeObjectMethod(Object object, String methodName, Object[] args, DragonBasicInterpreter dragonBasicInterpreter, CallStack callstack, SimpleNode callerInfo) throws ReflectException, EvalError, InvocationTargetException {
        // Dragon scripted object
        if (object instanceof This && !This.isExposedThisMethod(methodName)) {
            return ((This) object).invokeMethod(methodName, args, dragonBasicInterpreter, callstack, callerInfo, false/*delcaredOnly*/);
        }
    ...
    public static Object getIndex(Object array, int index) throws ReflectException, UtilTargetException {
        if (DragonBasicInterpreter.DEBUG) {
            DragonBasicInterpreter.debug("getIndex: " + array + ", index=" + index);
        }
        try {
    ...
    public static Method resolveJavaMethod(DragonClassManager bcm, Class clas, String name, Class[] types, boolean staticOnly) throws UtilEvalException {
        ...
        if (method == null) {
            ...
            if ((method != null) && !isPublic(method)) {
                if (publicOnly) {
                    DragonBasicInterpreter.debug("resolveJavaMethod - no accessible method found");
                    method = null;
                } else {
                    DragonBasicInterpreter.debug("resolveJavaMethod - setting method accessible");
                    try {
                        method.setAccessible(true);
                    } catch (final SecurityException e) {
                        DragonBasicInterpreter.debug("resolveJavaMethod - setting accessible failed: " + e);
                        method = null;
                    }
                }
            }
    ...
    private static Method findOverloadedMethod(final Class baseClass, final String methodName, final Class[] types, final boolean publicOnly) {
        if (DragonBasicInterpreter.DEBUG) {
            DragonBasicInterpreter.debug("Searching for method: " + StringUtil.methodString(methodName, types) + " in '" + baseClass.getName() + "'");
        }
    ...
    public static Object constructObject(Class clas, Object[] args) throws ReflectException, InvocationTargetException {
        ...
        if (DragonBasicInterpreter.DEBUG) {
            DragonBasicInterpreter.debug("Looking for most specific constructor: " + clas);
        }
    ...
    private static Method findMostSpecificMethod(final Class[] idealMatch, final Collection<Method> methods) {
        if (DragonBasicInterpreter.DEBUG) {
            DragonBasicInterpreter.debug("Looking for most specific method");
        }
     ...
     * methods must accept two additional leading arguments of the dragonBasicInterpreter
     * and callStack, respectively. e.g. invoke(dragonBasicInterpreter, callStack, ... )
    ...
    private static void logInvokeMethod(String msg, Method method, Object[] args) {
        if (DragonBasicInterpreter.DEBUG) {
            DragonBasicInterpreter.debug(msg + method + " with args:");
            for (int i = 0; i < args.length; i++) {
                final Object arg = args[i];
                DragonBasicInterpreter.debug("args[" + i + "] = " + arg + " type = " + (arg == null ? "<unkown>" : arg.getClass()));
            }
        }
    }
