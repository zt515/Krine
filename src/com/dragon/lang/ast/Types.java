...
/**
 * Static routines supporing type comparison and conversion in Dragon.
 * <p>
 * The following are notes on type comparison and conversion in Dragon.
 */
public class Types {
    /*
    ...
    */
    ...
    static final int DRAGON_ASSIGNABLE = 4;

    public static final int
            FIRST_ROUND_ASSIGNABLE = JAVA_BASE_ASSIGNABLE;
    public static final int LAST_ROUND_ASSIGNABLE = DRAGON_ASSIGNABLE;
    ...
    public static boolean isSignatureAssignable(Class[] from, Class[] to, int round) {
        ...
        switch (round) {
            ...
            case DRAGON_ASSIGNABLE:
                for (int i = 0; i < from.length; i++)
                    if (!isDragonAssignable(to[i], from[i]))
                        return false;
                return true;
            default:
                throw new InterpreterException("bad case");
        }
    }
     ...
     * <p/>
     * <p>
     * Note that the getAssignableForm() method is the primary dragon method for
     * checking assignability.  It adds additional dragon conversions, etc.
     *
     * @param lhsType assigning from rhsType to lhsType
     * @param rhsType assigning from rhsType to lhsType
     * @see #isDragonAssignable(Class, Class)
     */
    ...
    static boolean isJavaBaseAssignable(Class lhsType, Class rhsType) {
        /*
			Assignment to loose type, defer to dragon extensions
    ...
    /**
     * Test if a type can be converted to another type via Dragon
     * extended syntax rules (a superset of Java conversion rules).
     */
    static boolean isDragonAssignable(Class toType, Class fromType) {
        try {
            return castObject(
            ...
            throw new InterpreterException("error in cast check: " + e);
        }
    }

    /**
     * Attempt to cast an object instance to a new type if possible via
     * Dragon extended syntax rules.  These rules are always a superset of
     ...
     * @param fromValue an Object or dragon.Primitive primitive value (including
    ...
    public static Object castObject(
        ...
        if (fromValue == null)
            throw new InterpreterException("null fromValue");
     ...
     * respect to Dragon extended rules.  These rules are always a superset of
     ...
     * or cast is ultimately possible (with respect to Dragon) as well as the
     ...
     * in dragon is done on assignment as well as cast).
     * <p/>
     * <p>
	...
	*/
    private static Object castObject(
		...
		*/
        if (checkOnly && fromValue != null)
            throw new InterpreterException("bad cast params 1");
        if (!checkOnly && fromValue == null)
            throw new InterpreterException("bad cast params 2");
        if (fromType == Primitive.class)
            throw new InterpreterException("bad from Type, need to unwrap");
        ...
        if (toType == Void.TYPE)
            throw new InterpreterException("loose toType should be null");
        ...
        // Can we use the proxy mechanism to cast a dragon.This to
    ...
    /**
     * Return a UtilEvalException or UtilTargetException wrapping a ClassCastException
     * describing an illegal assignment or illegal cast, respectively.
     */
    static UtilEvalException castError(
