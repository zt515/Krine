 ...
 * com.dragon.lang commands.  However for normal use you should be using methods on
 * com.dragon.lang.DragonInterpreter to interact with your scripts.
 * <p>
 * <p>
 * A com.dragon.lang.ast.This object is a thin layer over a NameSpace that associates it with
 * an DragonInterpreter instance.  Together they comprise a Dragon scripted object
 * context.
 * <p>
 */
// not at all thread-safe - fschmidt
public class NameSpace implements Serializable, DragonClassManager.Listener, NameSource, Cloneable {

    private static final long serialVersionUID = 5004976946651004751L;

    public static final NameSpace JAVACODE =
            new NameSpace((DragonClassManager) null, "Called from compiled Java code.");
    ...
    private Map<String, List<DragonMethod>> methods;
    ...
    transient private DragonClassManager classManager;
	...
	*/
     ...
     * always absolute and are cached by DragonClassManager.
     */
    ...
    public NameSpace(DragonClassManager classManager, String name) {
        this(null, classManager, name);
    }

    public NameSpace(
            NameSpace parent, DragonClassManager classManager, String name) {
        // We might want to do this here rather than explicitly in DragonInterpreter
     ...
     */
    public Object get(String name, DragonBasicInterpreter dragonBasicInterpreter)
            throws UtilEvalException {
        CallStack callstack = new CallStack(this);
        return getNameResolver(name).toObject(callstack, dragonBasicInterpreter);
    }
     ...
     * this method outside of the com.dragon.lang package and wish to set variables with
     * primitive values you will have to wrap them using com.dragon.lang.ast.Primitive.
     *
     ...
     */
    public DragonMethod[] getMethods() {
        if (methods == null) {
            return new DragonMethod[0];
        } else {
            List<DragonMethod> ret = new ArrayList<DragonMethod>();
            for (List<DragonMethod> list : methods.values()) {
                ret.addAll(list);
            }
            return ret.toArray(new DragonMethod[0]);
        }
    }
     ...
     */
    public This getSuper(DragonBasicInterpreter declaringDragonBasicInterpreter) {
        if (parent != null)
            return parent.getThis(declaringDragonBasicInterpreter);
        else
            return getThis(declaringDragonBasicInterpreter);
    }

    /**
     * Get the top level namespace or this namespace if we are the top.
     * Note: this method should probably return type com.dragon.lang.ast.This to be consistent
     * with getThis();
     */
    public This getGlobal(DragonBasicInterpreter declaringDragonBasicInterpreter) {
        if (parent != null)
            return parent.getGlobal(declaringDragonBasicInterpreter);
        else
            return getThis(declaringDragonBasicInterpreter);
    }
    ...
    /**
     * A This object is a thin layer over a namespace, comprising a com.dragon.lang object
     * context.  It handles things like the interface types the com.dragon.lang object
     * supports and aspects of method invocation on it.
     * <p>
     * <p>
     * The declaringDragonBasicInterpreter is here to support callbacks from Java through
     * generated proxies.  The scripted object "remembers" who created it for
     * things like printing messages and other per-dragonBasicInterpreter phenomenon
	...
	/*
		...
		considering the declaringDragonBasicInterpreter could be different under some
		circumstances.  (Case: a child dragonBasicInterpreter running a source() / eval()
		command ).  However the effect is just that the main dragonBasicInterpreter that
		executes your script should be the one involved in call-backs from Java.

		I do not know if there are corner cases where a child dragonBasicInterpreter would
		...
		to find out... and if necessary we could cache on a per dragonBasicInterpreter
		...
		dragonBasicInterpreter, so we'd be sure to get the top dragonBasicInterpreter.
	*/
    public This getThis(DragonBasicInterpreter declaringDragonBasicInterpreter) {
        if (thisReference == null)
            thisReference = This.getThis(this, declaringDragonBasicInterpreter);
    ...
    public DragonClassManager getClassManager() {
        ...
        classManager = DragonClassManager.createClassManager(null/*interp*/);

        //DragonInterpreter.debug("No class manager namespace:" +this);
        return classManager;
    }

    void setClassManager(DragonClassManager classManager) {
...
// this?
            setClassManager(
                    DragonClassManager.createClassManager(null/*interp*/));
     ...
     * <p>
     * Note: this method is primarily intended for use internally.  If you use
     * this method outside of the com.dragon.lang package you will have to use
	...
	*/
     ...
     *
     * @see DragonBasicInterpreter#source(String)
     * @see DragonBasicInterpreter#eval(String)
     */
    public void setMethod(DragonMethod method)
            throws UtilEvalException {
        //checkMethodModifiers( method );

        if (methods == null)
            methods = new HashMap<String, List<DragonMethod>>();

        String name = method.getName();
        List<DragonMethod> list = methods.get(name);

        if (list == null) {
            methods.put(name, Collections.singletonList(method));
        } else {
            if (!(list instanceof ArrayList)) {
                list = new ArrayList<DragonMethod>(list);
     ...
     */
    public DragonMethod getMethod(String name, Class[] sig)
            throws UtilEvalException {
        return getMethod(name, sig, false/*declaredOnly*/);
    }

    /**
     * Get the com.dragon.lang method matching the specified signature declared in
     * this name space or a parent.
     * <p>
     * Note: this method is primarily intended for use internally.  If you use
     * this method outside of the com.dragon.lang package you will have to be familiar
     * with Dragon's use of the Primitive wrapper class.
     *
     ...
     *                     be visible.
     * @return the DragonMethod or null if not found
     * @see Primitive
     */
    public DragonMethod getMethod(
            String name, Class[] sig, boolean declaredOnly)
            throws UtilEvalException {
        DragonMethod method = null;
    ...
    /**
     * Import scripted or compiled Dragon commands in the following package
     * in the classpath.  You may use either "/" path or "." package notation.
     * e.g. importCommands("/com.dragon.lang/commands") or importCommands("com.dragon.lang.commands")
     ...
     * script the script is sourced into this namespace and the DragonMethod for
     ...
     * @return a DragonMethod, Class, or null if no such command is found.
    ...
    public Object getCommand(
        ...
        if (importedCommands != null) {
            ...
            for (int i = importedCommands.size() - 1; i >= 0; i--) {
                ...
                String scriptPath;
                if (path.equals("/"))
                    scriptPath = path + name + ".com.dragon.lang";
                else
                    scriptPath = path + "/" + name + ".com.dragon.lang";
                ...
                if (in != null)
                    return loadScriptedCommand(
                            in, name, argTypes, scriptPath, dragonBasicInterpreter);
        ...
        if (parent != null)
            return parent.getCommand(name, argTypes, dragonBasicInterpreter);
        else
            return null;
    }

    protected DragonMethod getImportedMethod(String name, Class[] sig)
            throws UtilEvalException {
        // Try object imports
        if (importedObjects != null)
            for (int i = 0; i < importedObjects.size(); i++) {
                ...
                if (method != null)
                    return new DragonMethod(method, object);
            }
        ...
        if (importedStatic != null)
            for (int i = 0; i < importedStatic.size(); i++) {
                ...
                if (method != null)
                    return new DragonMethod(method, null/*object*/);
            }
    ...
    /**
     * Load a command script from the input stream and find the DragonMethod in
	...
	*/
     ...
     * Qualified names are cached by the DragonClassManager.
    ...
    private Class getClassImpl(String name)
        ...
        if (DragonBasicInterpreter.DEBUG)
            DragonBasicInterpreter.debug("getClass(): " + name + " not	found in " + this);
        return null;
    }
    ...
    private Class getImportedClassImpl(String name)
        ...
        if (fullname != null) {
			...
			*/
            ...
            // (maybe this should happen in the DragonClassManager?)

            if (Name.isCompound(fullname))
                try {
                    clas = getNameResolver(fullname).toClass();
                } catch (ClassNotFoundException e) { /* not a class */ }
            else if (DragonBasicInterpreter.DEBUG) DragonBasicInterpreter.debug(
                    "imported unpackaged name not found:" + fullname);

            // If found cache the full name in the DragonClassManager
            if (clas != null) {
    ...
    */
     ...
     * dragonBasicInterpreter reference.  No caller information or call stack is
     * required.  The method will appear as if called externally from Java.
     * <p>
     *
     * @see This.invokeMethod(
     * String methodName, Object [] args, DragonBasicInterpreter interpreter,
     * CallStack callstack, SimpleNode callerInfo, boolean )
     */
    public Object invokeMethod(
            String methodName, Object[] args, DragonBasicInterpreter dragonBasicInterpreter)
            throws EvalError {
        return invokeMethod(
                methodName, args, dragonBasicInterpreter, null, null);
    }
    ...
    public Object invokeMethod(
            ...
            CallStack callstack, SimpleNode callerInfo)
            throws EvalError {
        return getThis(dragonBasicInterpreter).invokeMethod(
                methodName, args, dragonBasicInterpreter, callstack, callerInfo,
                false/*declaredOnly*/);
    }
     ...
     * <pre>
     * importClass("com.dragon.lang.ast.EvalError");
     * importClass("com.dragon.lang.DragonInterpreter");
     ...
     * importCommands("/com.dragon.lang/commands");
     * </pre>
     */
    public void loadDefaultImports() {
        ...
        importClass("com.dragon.lang.ast.EvalError");
        importClass("com.dragon.lang.DragonInterpreter");
        importClass("com.dragon.interpreter.DragonInterpreter");
        importPackage("java.net");
        importPackage("java.util");
        importPackage("java.io");
        importPackage("java.lang");
    ...
    /**
     * This is a helper method for working inside of com.dragon.lang scripts and commands.
     ...
     * com.dragon.lang.classpath.ClassIdentifier to use it );
     */
