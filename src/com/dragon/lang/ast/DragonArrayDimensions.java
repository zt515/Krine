 ...
 */
class DragonArrayDimensions extends SimpleNode {
    ...
    public Object eval(
            Class type, CallStack callstack, DragonBasicInterpreter dragonBasicInterpreter)
            throws EvalError {
        if (DragonBasicInterpreter.DEBUG) DragonBasicInterpreter.debug("array base type = " + type);
        baseType = type;
        return eval(callstack, dragonBasicInterpreter);
    }
    ...
    public Object eval(CallStack callstack, DragonBasicInterpreter dragonBasicInterpreter)
		...
		/*
			...
			e.g. int [][] { 1, 2 };
		*/
        if (child instanceof DragonArrayInitializer) {
            if (baseType == null)
                throw new EvalError(
                        "Internal Array Eval err:  unknown base type",
                        this, callstack);

            Object initValue = ((DragonArrayInitializer) child).eval(
                    baseType, numUndefinedDims, callstack, dragonBasicInterpreter);
