...
import com.dragon.lang.ast.Name;
...
/**
 * DragonClassManager manages all classloading in Dragon.
 * It also supports a dynamically loaded extension (dragon.classpath package)
 * which allows classpath extension and class file reloading.
 * <p>
 * Currently the extension relies on 1.2 for DragonClassLoader and weak
 * references.
 * <p>
 * See http://www.beanshell.org/manual/classloading.html for details
 * on the dragon classloader architecture.
 * <p>
 * <p>
...
/*
	...
	implementation (the optional dragon.classpath package) to accomodate all of
...
public class DragonClassManager {
    ...
    private static final Map<DragonClassManager, Object> classManagers = Collections.synchronizedMap(new WeakHashMap<DragonClassManager, Object>());

    public static void clearResolveCache() {
        DragonClassManager[] managers = classManagers.keySet().toArray(new DragonClassManager[0]);
        for (DragonClassManager m : managers) {
     ...
     * Class manager instnaces are now associated with the dragonBasicInterpreter.
     *
     * @see dragon.Interpreter.getClassManager()
     * @see dragon.Interpreter.setClassLoader( ClassLoader )
     */
    public static DragonClassManager createClassManager(DragonBasicInterpreter dragonBasicInterpreter) {
        DragonClassManager manager;

        // Do we have the optional package?
        if (Capabilities.classExists("com.dragon.lang.classpath.ClassManagerImpl"))
            try {
                // Try to load the module
                // don't refer to it directly here or we're dependent upon it
                Class clazz = Class.forName("com.dragon.lang.classpath.ClassManagerImpl");
                manager = (DragonClassManager) clazz.newInstance();
            } catch (Exception e) {
                throw new InterpreterException("Error loading ClassManager", e);
            }
        else
            manager = new DragonClassManager();

        if (dragonBasicInterpreter == null)
            dragonBasicInterpreter = new DragonBasicInterpreter();
        manager.declaringDragonBasicInterpreter = dragonBasicInterpreter;
     ...
     * See dragon.classpath.ClassManagerImpl for the fully functional class
     * management package.
     *
     ...
     * classloader.
     * If a DragonClassManager implementation is loaded the call will be
    ...
    public URL getResource(String path) {
        ...
        if (url == null)
            url = DragonBasicInterpreter.class.getResource(path);
    ...
    public InputStream getResourceAsStream(String path) {
        ...
        if (in == null)
            in = DragonBasicInterpreter.class.getResourceAsStream(path);
    ...
    /**
     * Clear the caches in DragonClassManager
     *
    ...
    /**
     * Set an external class loader.  Dragon will use this at the same
    ...
    public void dump(PrintWriter pw) {
        pw.println("DragonClassManager: no class manager.");
    }
    ...
    /*
		...
		namespace of the base name of classes.  i.e. Dragon cannot be in the
    ...
    public void definingClass(String className) {
        ...
        if (cur != null)
            throw new InterpreterException("Defining class problem: " + className
                    + ": Dragon cannot yet simultaneously define two or more "
	...
	/*
		...
		try {
			clazz = (Class)Reflect.invokeObjectMethod(
				...
				(DragonInterpreter)null, (CallStack)null, (SimpleNode)null
			);
		} catch ( Exception e ) {
			e.printStackTrace();
