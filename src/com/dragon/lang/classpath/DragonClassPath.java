...
import com.dragon.lang.ast.NameSource;
...
/**
 * A DragonClassPath encapsulates knowledge about a class path of URLs.
 * It can maps all classes the path which may include:
 * jar/zip files and base dirs
 * <p>
 * A DragonClassPath may composite other DragonClassPaths as components of its
...
public class DragonClassPath
    ...
    private List path;
    /**
     * Ordered list of components DragonClassPaths
     */
    ...
    public DragonClassPath(String name) {
        this.name = name;
        reset();
    }

    public DragonClassPath(String name, URL[] urls) {
        this(name);
        add(urls);
    }
    ...
    /**
     * Add the specified DragonClassPath as a component of our path.
     * Changes in the bcp will be reflected through us.
     */
    public void addComponent(DragonClassPath bcp) {
        if (compPaths == null)
    ...
    synchronized public Set getClassesForPackage(String pack) {
        ...
        if (compPaths != null)
            for (int i = 0; i < compPaths.size(); i++) {
                c = ((DragonClassPath) compPaths.get(i)).getClassesForPackage(
                        pack);
                if (c != null)
                    set.addAll(c);
            }
    ...
    synchronized public ClassSource getClassSource(String className) {
        ...
        if (cs == null && compPaths != null)
            for (int i = 0; i < compPaths.size() && cs == null; i++)
                cs = ((DragonClassPath) compPaths.get(i)).getClassSource(className);
        return cs;
    }
    ...
    protected synchronized void insureInitialized(boolean topPath) {
        ...
        if (compPaths != null)
            for (int i = 0; i < compPaths.size(); i++)
                ((DragonClassPath) compPaths.get(i)).insureInitialized(false);
    ...
    protected List getFullPath() {
        ...
        if (compPaths != null) {
            for (int i = 0; i < compPaths.size(); i++) {
                List l = ((DragonClassPath) compPaths.get(i)).getFullPath();
    ...
    */
    ...
    private UnqualifiedNameTable buildUnqualifiedNameTable() {
        ...
        if (compPaths != null)
            for (int i = 0; i < compPaths.size(); i++) {
                Set s = ((DragonClassPath) compPaths.get(i)).classSource.keySet();
...
*/
    ...
    public Set getPackagesSet() {
        ...
        if (compPaths != null)
            for (int i = 0; i < compPaths.size(); i++)
                set.addAll(
                        ((DragonClassPath) compPaths.get(i)).packageMap.keySet());
        return set;
    }
    ...
    static DragonClassPath userClassPath;

    /**
     * A DragonClassPath initialized to the user path
     * from java.class.path
     */
    public static DragonClassPath getUserClassPath()
            throws ClassPathException {
        if (userClassPath == null)
            userClassPath = new DragonClassPath(
                    "User Class Path", getUserClassPathComponents());
        return userClassPath;
    }

    static DragonClassPath bootClassPath;

    /**
     * Get the boot path including the lib/rt.jar if possible.
     */
    public static DragonClassPath getBootClassPath()
            throws ClassPathException {
        if (bootClassPath == null) {
            try {
                ...
                bootClassPath = new DragonClassPath(
    ...
    public static class JarClassSource extends ClassSource {
        ...
        /*
            ...
            DragonClassLoader can natively load from a JAR because it is a
    ...
    public String toString() {
        return "DragonClassPath " + name + "(" + super.toString() + ") path= " + path + "\n"
                + "compPaths = {" + compPaths + " }";
    }
