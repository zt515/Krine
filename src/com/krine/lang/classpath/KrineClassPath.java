...
import com.krine.lang.ast.NameSource;
...
/**
 * A KrineClassPath encapsulates knowledge about a class path of URLs.
 * It can maps all classes the path which may include:
 * jar/zip files and base dirs
 * <p>
 * A KrineClassPath may composite other KrineClassPaths as components of its
...
public class KrineClassPath
    ...
    public KrineClassPath(String name) {
        this.name = name;
        reset();
    }

    public KrineClassPath(String name, URL[] urls) {
        this(name);
        add(urls);
    }
    ...
    /**
     * Add the specified KrineClassPath as a component of our path.
     * Changes in the bcp will be reflected through us.
     */
    public void addComponent(KrineClassPath bcp) {
        if (compPaths == null)
...
*/
    ...
    static KrineClassPath userClassPath;

    /**
     * A KrineClassPath initialized to the user path
     * from java.class.path
     */
    public static KrineClassPath getUserClassPath()
            throws ClassPathException {
        if (userClassPath == null)
            userClassPath = new KrineClassPath(
                    "User Class Path", getUserClassPathComponents());
        return userClassPath;
    }

    static KrineClassPath bootClassPath;

    /**
     * Get the boot path including the lib/rt.jar if possible.
     */
    public static KrineClassPath getBootClassPath()
            throws ClassPathException {
        if (bootClassPath == null) {
            try {
                ...
                bootClassPath = new KrineClassPath(
    ...
    public static class JarClassSource extends ClassSource {
        ...
        /*
            ...
            KrineClassLoader can natively load from a JAR because it is a
    ...
    public String toString() {
        return "KrineClassPath " + name + "(" + super.toString() + ") path= " + path + "\n"
                + "compPaths = {" + compPaths + " }";
    }
